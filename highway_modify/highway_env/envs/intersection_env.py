...
class IntersectionEnv(AbstractEnv):
    ...
    def _make_road(self) -> None:
        ...
        left_turn_radius = right_turn_radius + lane_width * 2  # [m}
        ...
        for corner in range(4):
            ...
            # Incoming
            start1 = rotation @ np.array([lane_width / 2, access_length + lane_width + outer_distance])
            end1 = rotation @ np.array([lane_width / 2, outer_distance+ lane_width ])
            net.add_lane("o1" + str(corner), "ir1" + str(corner),
                         StraightLane(start1, end1, line_types=[s, s], priority=priority, speed_limit=10))
            start2 = rotation @ np.array([lane_width / 2 + lane_width, access_length + lane_width + outer_distance])
            end2 = rotation @ np.array([lane_width / 2 + lane_width, outer_distance + lane_width])
            net.add_lane("o2" + str(corner), "ir2" + str(corner),
                          StraightLane(start2, end2, line_types=[n, c], priority=priority, speed_limit=10))
            # Right turn
            r_center = rotation @ (np.array([outer_distance+lane_width, outer_distance + lane_width]))
            net.add_lane("ir2" + str(corner), "il2" + str((corner - 1) % 4),
            ...
            net.add_lane("ir1" + str(corner), "il1" + str((corner + 1) % 4),
            ...
            # Straight
            start = rotation @ np.array([lane_width / 2, outer_distance + lane_width])
            end = rotation @ np.array([lane_width / 2, -outer_distance - lane_width])
            net.add_lane("ir1" + str(corner), "il1" + str((corner + 2) % 4),
                         StraightLane(start, end, line_types=[n, n], priority=priority, speed_limit=10))
            # Exit
            start1 = rotation @ np.flip([lane_width / 2, access_length + outer_distance + lane_width], axis=0)
            end1 = rotation @ np.flip([lane_width / 2, outer_distance + lane_width], axis=0)
            net.add_lane("il1" + str((corner - 1) % 4), "o1" + str((corner - 1) % 4),
                         StraightLane(end1, start1, line_types=[n, s], priority=priority, speed_limit=10))
            start2 = rotation @ np.flip([lane_width / 2 + lane_width, access_length + outer_distance + lane_width], axis=0)
            end2 = rotation @ np.flip([lane_width / 2 + lane_width, outer_distance + lane_width], axis=0)
            net.add_lane("il2" + str((corner - 1) % 4), "o2" + str((corner - 1) % 4),
                         StraightLane(end2, start2, line_types=[n, c], priority=priority, speed_limit=10))
    ...
    def _make_vehicles(self, n_vehicles: int = 10) -> None:
        ...
        for _ in range(simulation_steps):
            [(self.road.act(), self.road.step(self.config["total_time"] / self.config["simulation_frequency"])) for _ in range(self.config["simulation_frequency"])]
        ...
        for ego_id in range(0, self.config["controlled_vehicles"]):
            if self.config["destination"] == "o3":
                ego_lane = self.road.network.get_lane(("o2{}".format(ego_id % 4), "ir2{}".format(ego_id % 4), 0))
            else:
                ego_lane = self.road.network.get_lane(("o1{}".format(ego_id % 4), "ir1{}".format(ego_id % 4), 0))
    ...
    def _spawn_vehicle(self,
        ...
        vehicle_type = utils.class_from_path(self.config["other_vehicles_type"])
        lane_id= random.randint(1,2)#TODO add !
        vehicle = vehicle_type.make_on_lane(self.road, ("o" + str(lane_id) + str(route[0]), "ir" + str(lane_id) + str(route[0]), 0),
