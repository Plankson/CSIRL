...
class RoundaboutLine2Env(AbstractEnv):
    ...
    def _reward(self, action: int) -> float:
        lane_change = 0#action == 0 or action == 2
    ...
    def _make_road(self) -> None:
        ...
        net = RoadNetwork()
        radii = [radius, radius+4, radius+8,radius+12]
        n, c, s = LineType.NONE, LineType.CONTINUOUS, LineType.STRIPED
        line = [[c, s], [n,c], [n,s], [n, c]]
        for lane in [0, 1]:
            net.add_lane("se", "ex",
...
register(
    id='roundabout-v0',
    entry_point='highway_env.envs:RoundaboutLine2Env',
)
